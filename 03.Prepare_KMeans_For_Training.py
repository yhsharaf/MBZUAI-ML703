...
for i in range(0, len(sort_array_txt)):
    if(sort_array_imgs[i].split("_")[1] == sort_array_txt[i].split("_")[1]):
        cluster_dict[sort_array_imgs[i].split("_")[1]] = [path+sort_array_txt[i],path+sort_array_imgs[i],'']
...
for n_cluster in n_clusters:
    ...
    for key in cluster_dict:
        with open(cluster_dict[key][0], "r") as f:
            ...
            next_line = next_line.replace(str(n_cluster)+"-KMeans-Cluster: ", "")
            # print(next_line)
    ...
    get_cluster_size = []
    get_cluster_size = getClusterSize(32,n_cluster,get_cluster_size)

    random_clusters_index = []
    for i in range(0,n_cluster):
        if(int(get_cluster_size[i])>len(cluster_key[str(i)])):
            random_clusters_index.append(np.random.choice(cluster_key[str(i)], size=get_cluster_size[i], replace=True))
        else:
            random_clusters_index.append(np.random.choice(cluster_key[str(i)], size=get_cluster_size[i], replace=False))

    for i in range(0,n_cluster):
        for j in range(0,len(random_clusters_index[i])):
            source_location_txt =  str(cluster_dict[random_clusters_index[i][j]][0]) 
            source_location_img =  str(cluster_dict[random_clusters_index[i][j]][1]) 
