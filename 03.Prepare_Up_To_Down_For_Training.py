...
folder_path = '/home/yhsharaf/Desktop/MBZUAI-ML703/BiwiDataset/faces_0/18'  # replace with the path to your folder
xyz_pattern = r"XYZ:\s*(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)"
xyz_dict = {}

# create the output folder if it does not exist
output_folder_path = '/home/yhsharaf/Desktop/MBZUAI-ML703/Up_Down'  # replace with the path to your output folder
...
for file_path in glob.glob(os.path.join(folder_path, '*.txt')):
    with open(file_path, 'r') as file:
        ...
        if match:
            ...
            if -90 <= x <= 90 and -8 <= y <= 8 and -8 <= z <= 8:
